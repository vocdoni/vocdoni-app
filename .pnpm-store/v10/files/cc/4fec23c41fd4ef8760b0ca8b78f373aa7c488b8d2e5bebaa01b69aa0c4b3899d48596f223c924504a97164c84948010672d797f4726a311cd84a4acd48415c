  ...
  return {
    ...
    getContentProps() {
      return normalize.element({
        ...
        "data-minimized": domQuery.dataAttr(isMinimized),
        "data-maximized": domQuery.dataAttr(isMaximized),
        "data-staged": domQuery.dataAttr(isStaged),
        style: {
          width: "var(--width)",
          height: "var(--height)",
  ...
  implementations: {
    ...
    effects: {
      ...
      trackBoundaryRect({ context, scope, prop, computed }) {
        ...
        const boundaryEl = prop("getBoundaryEl")?.();
        if (domQuery.isHTMLElement(boundaryEl)) {
          return domQuery.resizeObserverBorderBox.observe(boundaryEl, exec);
        }
...
exports.props = props;
exports.resizeTriggerAxes = resizeTriggerAxes;
