...
var anatomy = anatomy$1.createAnatomy("pagination").parts(
  "root",
  "item",
  "ellipsis",
  "firstTrigger",
...
var transform = (items) => {
  return items.map((value) => {
    if (utils.isNumber(value)) return { type: "page", value };
...
var getRange = (ctx) => {
  ...
  for (let i = 0; i < pages.length; i++) {
    if (pages[i] === ELLIPSIS) {
      const prevPage = utils.isNumber(pages[i - 1]) ? pages[i - 1] : 0;
      const nextPage = utils.isNumber(pages[i + 1]) ? pages[i + 1] : totalPages + 1;
