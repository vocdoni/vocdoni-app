...
function connect(service, normalize) {
  ...
  function getDecadeYears(year) {
    const range = dateUtils.getDecadeRange(startValue.year);
    return range.map((year2) => ({
      label: year2.toString(),
      value: year2,
      disabled: !utils.isValueWithinRange(year2, min?.year, max?.year)
    }));
  }
  ...
  function getYearTableCellState(props2) {
    ...
    const decadeYears = dateUtils.getDecadeRange(startValue.year, { strict: true });
    const isOutsideVisibleRange = !decadeYears.includes(value);
    const isOutsideRange = utils.isValueWithinRange(value, min?.year, max?.year);
    const cellState = {
    ...
    getMonthTableCellTriggerProps(props2) {
      ...
      return normalize.element({
        ...
        onPointerMove: isRangePicker ? (event) => {
          ...
          if (!cellState.selectable) return;
          const focus = !scope.isActiveElement(event.currentTarget);
          if (hoveredValue && cellState.value && date.isEqualMonth(cellState.value, hoveredValue)) return;
    ...
    getYearTableCellTriggerProps(props2) {
      ...
      return normalize.element({
        ...
        onPointerMove: isRangePicker ? (event) => {
          ...
          if (!cellState.selectable) return;
          const focus = !scope.isActiveElement(event.currentTarget);
          if (hoveredValue && cellState.value && date.isEqualYear(cellState.value, hoveredValue)) return;
  ...
  implementations: {
    ...
    actions: {
      ...
      announceValueText({ context, prop, refs }) {
        ...
        if (prop("selectionMode") === "range") {
          ...
          if (startDate && endDate) {
            announceText = dateUtils.formatSelectedDate(startDate, endDate, locale, timeZone);
          } else if (startDate) {
            announceText = dateUtils.formatSelectedDate(startDate, null, locale, timeZone);
          } else if (endDate) {
            announceText = dateUtils.formatSelectedDate(endDate, null, locale, timeZone);
          } else {
            announceText = "";
          }
        } else {
          announceText = value.map((date) => dateUtils.formatSelectedDate(date, null, locale, timeZone)).filter(Boolean).join(",");
        }
      ...
      selectParsedDate({ context, event, prop }) {
        ...
        if (!date) return;
        date = dateUtils.constrainValue(date, prop("min"), prop("max"));
        const values = Array.from(context.get("value"));
